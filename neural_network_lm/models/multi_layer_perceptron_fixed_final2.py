...
class MultiLayerPerceptron:
    ...
    def _create_training_data(self, words):
        ...
        # Check if we have enough words to create training data
        if len(words) <= self.context_size:
            raise ValueError(f"Input text is too short. Need more than {self.context_size} words.")
    ...
    def fit(self, text, progress_callback=None, stop_event=None):
        ...
        try:
            ...
            if len(words) <= self.context_size:
                error_msg = f"Input text is too short. Need more than {self.context_size} words."
                if progress_callback:
                    progress_callback(0, self.n_iterations, 0, 0, error_msg)
            ...
            if len(X) == 0 or len(y) == 0:
                error_msg = "No training data could be created. Text may be too short."
                if progress_callback:
                    progress_callback(0, self.n_iterations, 0, 0, error_msg)
                raise ValueError(error_msg)
            ...
            for iteration in range(self.n_iterations):
                ...
                try:
                    ...
                    error_msg = f"Error during training iteration {iteration}: {str(e)}\n{error_details}"
                    print(error_msg)
                    if progress_callback:
                        progress_callback(iteration, self.n_iterations, 0, 0, error_msg)
    ...
    def _forward(self, X):
        ...
        """
        # Check input shape
        if len(X.shape) == 3:  # If input has shape (batch_size, context_size, embedding_dim)
            # Reshape to (batch_size, context_size * embedding_dim)
            batch_size, context_size, embedding_dim = X.shape
            X = X.reshape(batch_size, context_size * embedding_dim)
    ...
    def predict_next_word(self, context):
        ...
        if not self.is_trained and (not self.vocabulary or len(self.vocabulary) == 0):
            info["error"] = "Model has not been trained yet"
            return self._fallback_prediction(context, info)
    ...
    def _fallback_prediction(self, context, info):
        """
        Generate a fallback prediction when the model encounters an error.
        ...
        # If we found a seed word, use it 30% of the time
        if seed_word and np.random.random() < 0.3:
            predicted_word = seed_word
            info["fallback"] = True
            info["fallback_source"] = "context_word"
        else:
            # Otherwise use a common word
            predicted_word = np.random.choice(common_words)
            info["fallback"] = True
            info["fallback_source"] = "common_word"
        
        # Add some fake probabilities
        info["probabilities"] = {
            predicted_word: 0.5,
            common_words[0]: 0.1,
            common_words[1]: 0.1,
    ...
    def _predict_next_word_with_temperature(self, context, temperature=1.0):
        ...
        # Check if model has been trained
        if not self.is_trained and (not self.vocabulary or len(self.vocabulary) == 0):
            info = {"error": "Model has not been trained yet", "probabilities": {}}
            return self._fallback_prediction(context, info)
            
        # Get prediction and info
        word, info = self.predict_next_word(context)
        
        # If there was an error or fallback, return it
    ...
    def predict_next_n_words(self, context, n=5, temperature=1.0):
        ...
        # Check if model has been trained
        if not self.is_trained and (not self.vocabulary or len(self.vocabulary) == 0):
            prediction_info["error"] = "Model has not been trained yet. Using fallback generation."
            ...
            for i in range(n):
                ...
                step_info = {
                    ...
                    "note": "Fallback word (model not trained)",
                    "fallback": True
                }
        ...
        for i in range(n):
            try:
                ...
                # If we got a fallback, increment error count
    ...
    def save_for_huggingface(self, directory):
        """
        ...
        if self.weights and self.biases:
            ...
            for i, (weight, bias) in enumerate(zip(self.weights, self.biases)):
                np.save(os.path.join(weights_dir, f'weight_{i}.npy'), weight)
                np.save(os.path.join(weights_dir, f'bias_{i}.npy'), bias)
        ...
        if self.tokenizer:
            ...
            if hasattr(self.tokenizer, 'get_vocabulary'):
                tokenizer_vocab = self.tokenizer.get_vocabulary()
        ...
        readme_content = f"""# Multi-Layer Perceptron Language Model
...
- **Vocabulary Size**: {len(self.vocabulary) if self.vocabulary else 0}
...
"""
        ...
        # Create a simple Python script to load and use the model
...
"""
        ...
        with open(os.path.join(directory, 'load_model.py'), 'w') as f:
            f.write(script_content)
        
        print(f"Model saved in Hugging Face format at: {directory}")
        return directory
    ...
    @classmethod
    def from_huggingface(cls, model_path):
        """
        Load a model from Hugging Face format.
        ...
        model = cls(
            ...
            tokenizer_type=config.get('tokenizer_type', 'wordpiece'),
        ...
        # Set output size
        model.output_size = len(model.vocabulary)
        ...
        # Set input size
        model.input_size = model.context_size * model.embedding_dim
