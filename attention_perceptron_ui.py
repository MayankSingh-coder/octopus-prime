...
if TKINTER_AVAILABLE:
    class AttentionPerceptronUI(MultiLayerPerceptronUI):
        ...
        def __init__(self, root):
            ...
            # Find the 'Model Parameters' frame in the base class UI
            # Use the instance variable from the base class
            params_frame = self.params_frame
                 
            if params_frame: # Check if the frame exists
                      # Add model type selection
                      model_type_frame = ttk.Frame(params_frame)
                      if params_frame.winfo_children():
                      ...
                      # Simplify packing: Remove 'after' argument
                      ...
                      if info_frame: # Check if the frame exists
                            # Add model type info
                            ...
                            attention_info_frame.pack(fill=tk.X, pady=5) # Add this line to pack the frame
                            ttk.Label(attention_info_frame, text="Attention Info:").pack(side=tk.LEFT)
